...
try {
  ...
  const mapLayout = await evaluate(`(()=>{const frame=document.fullscreenElement,stage=frame?.querySelector('.map-stage'),search=frame?.querySelector('.map-toolbar-search'),activity=frame?.querySelector('.toolbar-activity-filter'),button=frame?.querySelector('.map-fullscreen-button.active');if(!frame||!stage||!search||!activity||!button)return null;const s=stage.getBoundingClientRect(),q=search.getBoundingClientRect(),a=activity.getBoundingClientRect();return{stageWidth:s.width,stageHeight:s.height,searchWidth:q.width,activityWidth:a.width,buttonLabel:button.getAttribute('aria-label')}})()`);
  ...
  const intelligenceLayout = await evaluate(`(()=>{const frame=document.fullscreenElement,map=frame?.querySelector('.real-map'),tabs=frame?.querySelector('.election-switch'),layers=frame?.querySelector('.layer-switch'),button=frame?.querySelector('.map-fullscreen-button.active');if(!frame||!map||!tabs||!layers||!button)return null;const m=map.getBoundingClientRect(),t=tabs.getBoundingClientRect(),l=layers.getBoundingClientRect();const tabText=tabs.innerText||'';const layerText=layers.innerText||'';return{mapWidth:m.width,mapHeight:m.height,tabsVisible:t.width>0&&t.height>0,layersVisible:l.width>0&&l.height>0,allElectionTabs:['Presidencia','Lista nacional','Distrito','Alcaldía','Parlacen'].every((label)=>tabText.includes(label)),allLayerControls:['Ganador','Participación','Margen','Actas','Electoral','Escuelas','CEM','Salud','Obras'].every((label)=>layerText.includes(label)),buttonLabel:button.getAttribute('aria-label')}})()`);
  const intelligenceScreenshotBytes = await capture("inteligencia-mapa-fullscreen");
  if (!intelligenceLayout || intelligenceLayout.mapWidth < 700 || intelligenceLayout.mapHeight < 450 || !intelligenceLayout.tabsVisible || !intelligenceLayout.layersVisible || !intelligenceLayout.allElectionTabs || !intelligenceLayout.allLayerControls || intelligenceLayout.buttonLabel !== "Salir de pantalla completa" || intelligenceScreenshotBytes < 10_000) throw new Error(`Intelligence fullscreen failed for ${municipalityCode}: ${JSON.stringify(intelligenceLayout)}`);
  interactions.push({ kind: "intelligence-fullscreen", ...intelligenceLayout, screenshotBytes: intelligenceScreenshotBytes, ok: true });
  await cdp.send("Input.dispatchKeyEvent", { type: "keyDown", key: "Escape", code: "Escape", windowsVirtualKeyCode: 27 });
  await cdp.send("Input.dispatchKeyEvent", { type: "keyUp", key: "Escape", code: "Escape", windowsVirtualKeyCode: 27 });
