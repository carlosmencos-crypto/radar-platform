...
try {
  ...
  await delay(500);
  const intelligenceFullscreen = await evaluate(`(()=>{const frame=document.fullscreenElement,map=frame?.querySelector('.real-map'),button=frame?.querySelector('.map-fullscreen-button.active'),directory=frame?.querySelector('.directory'),list=frame?.querySelector('.center-list'),note=frame?.querySelector('.directory-note');if(!frame||!map||!button||!directory||!list||!note)return null;const m=map.getBoundingClientRect(),b=button.getBoundingClientRect(),d=directory.getBoundingClientRect(),l=list.getBoundingClientRect(),n=note.getBoundingClientRect();return{map:{width:m.width,height:m.height},button:{width:b.width,height:b.height,label:button.getAttribute('aria-label')},directory:{height:d.height,bottom:d.bottom},list:{height:l.height,bottom:l.bottom},note:{top:n.top,bottom:n.bottom},unusedBottom:d.bottom-n.bottom};})()`);
  if (!intelligenceFullscreen || intelligenceFullscreen.map.width < 700 || intelligenceFullscreen.map.height < 500 || intelligenceFullscreen.button.width > 100 || intelligenceFullscreen.button.label !== "Salir de pantalla completa" || intelligenceFullscreen.list.height < 600 || intelligenceFullscreen.note.top-intelligenceFullscreen.list.bottom > 20 || intelligenceFullscreen.unusedBottom > 24) throw new Error(`Intelligence Map fullscreen layout is unusable: ${JSON.stringify(intelligenceFullscreen)}`);
