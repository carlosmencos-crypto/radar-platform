create table if not exists private.stg_voter_roll_community_aggregates (
  batch_id text not null,
  municipality_code text not null,
  source_year integer not null,
  community_normalized text not null,
  community_label text,
  elector_count integer not null,
  average_age_base numeric,
  age_18_29 integer not null,
  age_30_44 integer not null,
  age_45_59 integer not null,
  age_60_plus integer not null,
  age_unclassified_count integer not null default 0,
  source_row_hash text,
  staged_at timestamptz not null default now(),
  primary key (batch_id, municipality_code, source_year, community_normalized)
);

revoke all on private.stg_voter_roll_community_aggregates from anon, authenticated;
...
begin
  ...
  insert into data_vault.voter_roll_community_aggregates (
    municipality_id, source_year, community_normalized, community_label,
    elector_count, average_age_base,
    age_18_29, age_30_44, age_45_59, age_60_plus,
    age_unclassified_count, coverage_band, updated_at
  )
  select
    m.id, s.source_year, s.community_normalized, s.community_label,
    s.elector_count, s.average_age_base,
    s.age_18_29, s.age_30_44, s.age_45_59, s.age_60_plus,
    s.age_unclassified_count, 'COMMUNITY', now()
  from private.stg_voter_roll_community_aggregates s
  join public.municipalities m on m.municipality_code = s.municipality_code
  where s.batch_id = p_batch_id
  ...
  from (
    ...
    join data_vault.voter_roll_municipal_aggregates a
      on a.municipality_id = m.id and a.source_year = s.source_year
    left join data_vault.voter_roll_community_aggregates t
      on t.municipality_id = m.id and t.source_year = s.source_year
    where s.batch_id = p_batch_id
    group by s.municipality_code, s.source_year,
      a.community_count, a.elector_count,
      a.age_18_29, a.age_30_44, a.age_45_59, a.age_60_plus, a.age_missing_count
    having count(t.*) <> a.community_count
       or coalesce(sum(t.elector_count), 0) <> a.elector_count
       or coalesce(sum(t.age_18_29), 0) <> a.age_18_29
       or coalesce(sum(t.age_30_44), 0) <> a.age_30_44
       or coalesce(sum(t.age_45_59), 0) <> a.age_45_59
       or coalesce(sum(t.age_60_plus), 0) <> a.age_60_plus
       or coalesce(sum(t.age_unclassified_count), 0) <> a.age_missing_count
  ) q;
