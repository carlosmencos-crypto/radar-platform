...
test("report retains every validated entry, unknown magnitudes and unit caveats", () => {
  for (const code of ["0501", "0502", "0509", "0513", "0101", "0103", "0107", "0113", "0114", "1208"]) {
...
test("recovered Vault products bind every official municipality without inventing missing values", () => {
  ...
  assert.equal(rows.reduce((n, r) => n + r.planning.priorities.length, 0), 104);
  assert.equal(rows.reduce((n, r) => n + r.planning.indicators.length, 0), 12);
  assert.equal(rows.filter((r) => r.planning.review_status === "PARTIAL_VALIDATED_CONTENT").length, 10);
});
...
test("direct PDF reviews remain historical, source-bound, and incomplete", () => {
  for (const code of ["0102", "0103", "0107", "0113", "0114", "1208"]) {
    const row = get(code);
    ...
    assert.equal(plan.document.source_qa, ["0103", "0107", "0113", "0114"].includes(code) ? "PASS_SOURCE_READINESS" : "FAIL_CLOSED");
