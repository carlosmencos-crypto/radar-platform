...
export function V70MunicipalPlanning({ model }: { model: MunicipalIntelligenceModel }) {
  ...
  return <div className="municipal-planning-depth">
    ...
    {hasContent ? <p className="trace-note">{planning.priorities.length ? `${planning.priorities.length} prioridades` : `${planning.indicators.length} indicadores`} recuperados de productos previamente validados. La revisión integral del plan permanece pendiente. {Array.from(new Map([...planning.priorities, ...planning.indicators].map((item) => [item.product_url, item])).values()).map((item) => <a key={item.product_url} href={item.product_url} target="_blank" rel="noreferrer">Consultar producto validado ↗ </a>)}</p> : null}
