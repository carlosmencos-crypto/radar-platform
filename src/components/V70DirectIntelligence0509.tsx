...
function CanonicalMunicipalIntelligenceContent({ onSelectionChange }: { onSelectionChange: (selection: { electionCode: V70ElectionCode; centerId: string }) => void }) {
  ...
  return <>
    ...
    <section className="kpis" aria-label="Indicadores principales">
      <article><small>{model.populationOfficialBasis ? "Población base TSE 2027" : `Población proyectada ${model.projectionYear ?? 2026}`}</small><b>{projection === null ? "—" : integer.format(projection)}</b><em>{model.populationSource ?? "INE · no publicado"}</em></article>
