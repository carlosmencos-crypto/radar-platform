...
import {
  canViewRadarSection,
  type RadarRuntimeSection,
} from "../data/radarSectionAccess";
...
type PortalControlsProps = { theme: RadarTheme; textSize: RadarTextSize; onExport: () => void; onTextSize: () => void; onTheme: () => void };
...
function profileState(status: "validated" | "partial" | "pending"): AvailabilityState { return status === "validated" ? "disponible" : status === "partial" ? "parcial" : "pendiente"; }
...
function PublicHome() {
  ...
  const visibleTotal = consumer.modules.filter((module) => module.frontend_action !== "HIDE_POST_LAUNCH").length;
  const available = runtime?.layers.length ?? consumer.modules.filter((module) => module.vault === "data" && module.state === "disponible" && module.frontend_action !== "HIDE_POST_LAUNCH").length;
  const readiness = runtime?.client_readiness;
  const readinessLabel = readiness?.status === "CLIENT_READY"
    ? "ENTORNO CLIENTE LISTO"
    : readiness?.status === "INTELLIGENCE_READY"
      ? "INTELIGENCIA LISTA"
      : "CONFIGURACIÓN EN VALIDACIÓN";
  const campaignLabel = runtime?.context.campaign_name ?? "Campaña no configurada";
  const campaignDetail = readiness?.campaign_connected
    ? "Campaign Vault autorizado"
    : "Campaign Vault listo para asociar";
  ...
  return <>
    <section className="command-hero home-welcome" data-client-readiness={readiness?.status ?? "BLOCKED"}><div className="home-welcome-copy"><h3>HOLA</h3><small>CENTRO DE CONTROL ELECTORAL 2027</small><h1>{municipality}</h1><span className="campaign-type">{readinessLabel}</span></div><aside className="campaign-identity" aria-label="Identidad de campaña"><div className="candidate-photo-wrap"><i aria-label="Perfil de campaña">—</i><span>PERFIL DE CAMPAÑA</span></div><div className="candidate-copy"><small>CANDIDATO A LA ALCALDÍA</small><b>{campaignLabel}</b><span>Contexto municipal · {municipality}</span><div className="party-signature"><span className="party-logo-button"><i>LOGO</i></span><span><small>ORGANIZACIÓN POLÍTICA</small><strong>{campaignDetail}</strong></span></div></div></aside></section>
    <section className="home-reminder-bar" aria-label="Resumen del municipio"><div><small>{today}</small></div><div className="territory-coverage"><small>DATA VAULT MUNICIPAL</small><b>{available}/{visibleTotal}</b><i>{readinessLabel}</i></div><Link to={routeFor(consumer.municipality.code, "inteligencia")}>Revisar inteligencia →</Link></section>
...
function Intelligence() {
  ...
  const womenShare = sexTotal > 0 && women !== undefined ? women / sexTotal * 100 : 0; const menShare = sexTotal > 0 && men !== undefined ? men / sexTotal * 100 : 0;
  const sexGap = women !== undefined && men !== undefined ? Math.abs(women - men) : undefined; const maxAgeShare = Math.max(...(intelligence?.ages.map((item) => item.share) ?? [1]), 1);
  const geoTypes = useMemo(() => new Set(Object.keys(featurePresentation).filter((type) => (geoBundle?.feature_counts[type] ?? 0) > 0)), [geoBundle]);
  const insightModules = profile?.modules.filter((module) => ["territorio", "educacion", "salud", "finanzas", "obras"].includes(module.id)) ?? [];
  const visibleModules = consumer.modules.filter((module) => module.vault === "data" && module.frontend_action !== "HIDE_POST_LAUNCH");

  const traceability = <details className="canonical-coverage-secondary"><summary><span><small>FUENTES Y TRAZABILIDAD</small><b>Detalle institucional del expediente</b></span><Status state={overallState} /></summary><section className="module-card-grid canonical-module-grid">{visibleModules.map((module) => { const detail = profile?.modules.find((item) => item.id === module.ui_profile_id); return <article key={module.id}><Status state={module.state === "pendiente" && detail?.metrics.length ? "disponible" : module.state} /><h2>{module.label}</h2><p>{detail?.summary ?? "Cobertura oficial conectada al expediente municipal."}</p><small className="canonical-source">{detail?.source ?? module.source ?? "RADAR Data Vault"}</small></article>; })}</section></details>;
  ...
  return <>
    ...
    {intelligence ? <>
      <section className="kpis" aria-label="Indicadores principales"><article><small>Población proyectada 2026</small><b>{intelligence.populationProjection || "—"}</b><em>INE · proyección oficial al 30 de junio</em></article><article><small>Padrón electoral activo 2026</small><b>{intelligence.voterRegister}</b><em>{intelligence.voterWomen ? `${intelligence.voterWomen} mujeres` : "TSE"}{intelligence.voterMen ? ` · ${intelligence.voterMen} hombres` : ""}</em></article><article><small>Centros electorales geolocalizados</small><b>{intelligence.votingCenters || "—"} <i>centros</i></b><em>{intelligence.votingBoards ? `${intelligence.votingBoards} JRV · ` : ""}TSE 2023</em></article><article><small>Organización territorial</small><b>{intelligence.communityRecords || "—"} <i>comunidades</i></b><em>{intelligence.territorialGroups ? `${intelligence.territorialGroups} agrupaciones territoriales` : "Núcleo electoral municipal"}</em></article></section>
      <section className="section electorate-profile"><div className="section-head"><div><p className="eyebrow">PERFIL DEL ELECTORADO</p><h2>Quiénes pueden votar hoy</h2></div><p>RADAR mantiene separados padrón activo, padrón detallado, Censo y proyecciones para evitar cruces de universos incompatibles.</p></div><div className="electorate-hero"><article className="register-total"><span>PADRÓN ACTIVO 2026</span><b>{intelligence.voterRegister}</b><p>Corte oficial: {intelligence.registerCut || "2026"}</p>{intelligence.registerGrowth ? <div><strong>{intelligence.registerGrowth}</strong><small>personas frente al padrón oficial 2023<br />variación: {intelligence.registerGrowthRate}</small></div> : null}</article>{women !== undefined && men !== undefined ? <article className="sex-profile"><div className="profile-title"><span>COMPOSICIÓN POR SEXO</span>{sexGap !== undefined ? <b>Brecha: {new Intl.NumberFormat("es-GT").format(sexGap)}</b> : null}</div><div className="split-meter"><i style={{ width: `${womenShare}%` }} /><em style={{ width: `${menShare}%` }} /></div><div className="split-labels"><span><i />Mujeres <b>{intelligence.voterWomen}</b><small>{womenShare.toFixed(1)}%</small></span><span><i />Hombres <b>{intelligence.voterMen}</b><small>{menShare.toFixed(1)}%</small></span></div></article> : null}{intelligence.literacyRate ? <article className="literacy-profile"><div><span>ALFABETISMO REGISTRADO</span><b>{intelligence.literacyRate}</b>{intelligence.literatePeople ? <small>{intelligence.literatePeople} personas</small> : null}</div><div className="literacy-detail">{intelligence.womenLiteracy ? <span>Mujeres <b>{intelligence.womenLiteracy}</b></span> : null}{intelligence.menLiteracy ? <span>Hombres <b>{intelligence.menLiteracy}</b></span> : null}{intelligence.literacyUnregistered ? <span>Sin alfabetismo registrado <b>{intelligence.literacyUnregistered}</b></span> : null}</div></article> : null}</div><div className="age-and-territory">{intelligence.ages.length ? <article className="age-profile"><div className="profile-title"><span>ESTRUCTURA POR EDAD</span><b>Padrón detallado · universo separado</b></div><div className="age-bars">{intelligence.ages.map((item) => <div key={item.label}><span>{item.label}</span><i><em style={{ width: `${item.share / maxAgeShare * 100}%` }} /></i><b>{item.value}</b><small>{item.share.toFixed(1)}%</small></div>)}</div></article> : null}<article className="universe-card"><div className="profile-title"><span>POBLACIÓN Y TERRITORIO</span><b>Universos separados</b></div><div className="universe-block current"><span>TSE · PADRÓN 2026</span><b>{intelligence.voterRegister}</b><small>Ciudadanos empadronados activos.</small></div>{intelligence.censusPopulation ? <div className="universe-block census"><span>INE · CENSO 2018</span><b>{intelligence.censusPopulation}</b><div className="rural-bar"><i style={{ width: `${intelligence.censusUrbanShare}%` }} /><em style={{ width: `${intelligence.censusRuralShare}%` }} /></div><p><strong>{intelligence.censusUrban} urbanos · {intelligence.censusUrbanShare.toFixed(1)}%</strong><strong>{intelligence.censusRural} rurales · {intelligence.censusRuralShare.toFixed(1)}%</strong></p></div> : null}{intelligence.populationProjection ? <div className="universe-block projection"><span>INE · PROYECCIÓN 2026</span><b>{intelligence.populationProjection}</b><small>{intelligence.projectionMen ? `${intelligence.projectionMen} hombres` : ""}{intelligence.projectionMen && intelligence.projectionWomen ? " · " : ""}{intelligence.projectionWomen ? `${intelligence.projectionWomen} mujeres` : ""}. Proyección poblacional, no padrón.</small></div> : null}</article></div></section>
    </> : null}

    {geoBundle && runtime?.geo.bbox ? <section className="section"><div className="section-head"><div><p className="eyebrow">LECTURA TERRITORIAL</p><h2>El municipio sobre el mapa</h2></div><p>Lugares poblados, centros de votación, educación y salud comparten la misma cartografía municipal.</p></div><section className="smart-map-shell map-v3"><div className="map-stage"><RuntimeSynchronizedMap municipalityName={municipality_name} features={geoBundle.features} activeTypes={geoTypes} query="" bbox={runtime.geo.bbox} satellite={false} /><aside className="map-electoral-priorities"><header><small>COBERTURA TERRITORIAL</small><div><b>Data Vault</b><span>{geoBundle.features.length} puntos</span></div></header>{Object.keys(featurePresentation).filter((type) => geoTypes.has(type)).map((type) => { const item = featurePresentation[type]; return <div key={type}><span><b>{item.label.toUpperCase()}</b><small>{item.detail}</small></span><em>{geoBundle.feature_counts[type] ?? 0}</em></div>; })}</aside></div></section><Link to={routeFor(consumer.municipality.code, "mapa")}>Abrir Mapa Inteligente →</Link></section> : null}

    {insightModules.length ? <section className="section"><div className="section-head"><div><p className="eyebrow">EXPEDIENTE MUNICIPAL 360</p><h2>Lo que define el municipio</h2></div><p>Territorio, servicios, finanzas e inversión reunidos sin mezclar períodos ni universos.</p></div><section className="module-card-grid canonical-module-grid">{insightModules.map((module) => <article key={module.id}><Status state={profileState(module.status)} /><h2>{module.title}</h2><p>{module.summary}</p>{module.metrics.slice(0, 6).map((item) => <div className="canonical-metric" key={item.label}><b>{item.value}</b><span>{item.label} · {item.detail}</span></div>)}</article>)}</section></section> : null}

    <p className="trace-note"><Status state="disponible" /> RADAR prioriza la información oficial disponible y mantiene cada fuente, período y universo separados. Las excepciones metodológicas quedan documentadas en trazabilidad.</p>
    {traceability}
  </>;
}
...
function MunicipalDashboardShell() {
  ...
  const accessContext = { campaignId: campaign_id, userRole: user_role, permissions };
  const visibleSections = sections.filter(([, , slug]) =>
    canViewRadarSection(slug, accessContext),
  );
  ...
  const title = selected[1]; const eyebrow = selected[3]; const name = `${municipality_name} · ${department_name}`;
  const accountLabel = user_role === "national_admin" ? "Acceso nacional" : "Sesión autorizada";
  const accountDetail = user_role === "national_admin" ? `${municipality_code} · Data Vault` : `${municipality_code} · ${user_role}`;
