...
function PublicHome() {
  ...
  return <>
    <section className="command-hero home-welcome"><div className="home-welcome-copy"><h3>HOLA</h3><small>CENTRO DE CONTROL ELECTORAL 2027</small><h1>{municipality}</h1><span className="campaign-type">Campaña Alcaldía</span></div><aside className="campaign-identity" aria-label="Identidad de campaña"><div className="candidate-photo-wrap"><i aria-label="Perfil de campaña">—</i><span>PERFIL DE CAMPAÑA</span></div><div className="candidate-copy"><small>CANDIDATO A LA ALCALDÍA</small><b>Campaña no configurada</b><span>Contexto municipal · {municipality}</span><div className="party-signature"><span className="party-logo-button"><i>LOGO</i></span><span><small>ORGANIZACIÓN POLÍTICA</small><strong>Campaign Vault listo para asociar</strong></span></div></div></aside></section>
    <section className="home-reminder-bar" aria-label="Resumen del municipio"><div><small>{today}</small></div><div className="territory-coverage"><small>DATA VAULT MUNICIPAL</small><b>{available}/{visibleTotal}</b><i>RADAR</i></div><Link to={routeFor(consumer.municipality.code, "inteligencia")}>Revisar inteligencia →</Link></section>
