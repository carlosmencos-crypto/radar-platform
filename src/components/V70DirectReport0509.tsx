...
const base = "/municipio/0509";
const common = { municipality: "San José / Puerto San José", department: "Escuintla", code: "0509", campaignType: "Campaña Alcaldía" } as const;
const definitions: Record<string, ReportDefinition> = {
  inicio: { key:"inicio", eyebrow:"CENTRO DE CONTROL ELECTORAL 2027", title:"Resumen ejecutivo de campaña", subtitle:"San José / Puerto San José · Campaña Alcaldía", summary:"Lectura general del proyecto político, la planilla municipal y las tareas que requieren atención.", vault:"CAMPAIGN VAULT", backHref:base, metrics:[{label:"Integrantes de planilla",value:"12",note:"3 síndicos · 8 concejales"},{label:"Expedientes completos",value:"0",note:"Control legal preliminar"},{label:"Expedientes pendientes",value:"12",note:"Incluye documentos en revisión"},{label:"Municipio",value:"0509",note:"San José · Escuintla"}], sections:[{eyebrow:"PLANILLA MUNICIPAL",title:"Integración de la candidatura",items:["Candidato a alcalde pendiente","Síndicos I–III: expedientes pendientes","Concejales I–VIII: expedientes pendientes"]},{eyebrow:"CONTROL EJECUTIVO",title:"Módulos conectados",items:["Inteligencia Municipal y Mapa Inteligente","Campaña, Agenda y Directorio","Legal, recursos, encuestas y control financiero"]}], sources:"Perfil de campaña y Campaign Vault · información de ejemplo pendiente de personalización administrativa." },
  "municipio-360": { key:"municipio-360", eyebrow:"EXPEDIENTE MUNICIPAL 360", title:"Inteligencia Municipal", subtitle:"Fotografía estratégica del municipio para definir mensajes y prioridades", summary:"Documento ejecutivo construido con fuentes públicas trazables, conservando el año y alcance territorial de cada dato.", vault:"DATA VAULT", backHref:`${base}/inteligencia`, metrics:[{label:"Población proyectada 2026",value:"72,156",note:"INE · proyección oficial"},{label:"Padrón activo 2026",value:"40,890",note:"21,531 mujeres · 19,359 hombres"},{label:"Centros electorales",value:"13",note:"103 JRV enlazadas"},{label:"Lugares poblados INE",value:"82",note:"81 con coordenadas"}], sections:[{key:"electoral",eyebrow:"LECTURA ELECTORAL",title:"Historial municipal",items:["Memorias oficiales procesadas: 2011, 2015, 2019 y 2023.","Ganadora municipal 2023: Odilia, partido VALOR.","TREP 2023 se conserva rotulado como preliminar y separado de la Memoria Electoral."]},{key:"center",eyebrow:"CENTRO SELECCIONADO",title:"Lectura centro por centro",items:["13 centros únicos y 103 JRV enlazadas.","38,934 electores 2023 enlazados a centros.","La ficha conserva electores, JRV, tipo y cobertura de actas."]},{key:"territory",eyebrow:"TERRITORIO",title:"Organización municipal",items:["4 microrregiones y 5 centralidades según PDM-OT.","20 COCODES de primer nivel y 25 de segundo nivel.","82 lugares INE: 81 con coordenadas y uno sin coordenadas."]},{key:"indicators",eyebrow:"INDICADORES MUNICIPALES",title:"Población e infraestructura",items:["62,801 habitantes en el Censo 2018; proyección 2026 de 72,156.","5 establecimientos MSPAS en la capa nacional validada.","76 registros educativos geoespaciales; la capa continúa siendo parcial."]},{key:"finance",eyebrow:"FINANZAS Y GESTIÓN",title:"Presupuesto, inversión y obras",items:["Ingresos municipales disponibles para 2016-2025.","Egresos comparables 2016-2023; Región V 2024 continúa pendiente.","7 obras municipales y 20 proyectos SNIP se mantienen como universos separados."]},{eyebrow:"REGLA DE DATOS",title:"Límites de interpretación",items:["Los vacíos nunca se convierten en cero.","No se mezclan escalas municipales, departamentales y nacionales.","No se suman universos de MINFIN, SNIP, CODEDE, municipalidad y Guatecompras."]}], sources:"TSE · INE · MINFIN · SEGEPLAN · MINEDUC · MSPAS · Guatecompras · CONRED." },
...
function reportCode(definition:ReportDefinition){return `RADAR-${common.code}-${definition.key.toUpperCase().replace(/[^A-Z0-9]+/g,"-")}`;}
...
function PageHeader({definition}:{definition:ReportDefinition}){return <header className="report-page-header"><Brand/><div><b>{definition.title}</b><span>{common.municipality} · {common.code}</span></div></header>}
function PageFooter({definition,page,total}:{definition:ReportDefinition;page:number;total:number}){return <footer className="report-page-footer"><span>{definition.vault} · {reportCode(definition)}</span><b>{page} / {total}</b></footer>}
function Sheet({definition,page,total,children,cover=false}:{definition:ReportDefinition;page:number;total:number;children:React.ReactNode;cover?:boolean}){return <section className={`report-sheet ${cover?"cover":""} ${definition.landscape&&!cover?"landscape":""}`}>{!cover?<PageHeader definition={definition}/>:null}<div className="report-page-body">{children}</div><PageFooter definition={definition} page={page} total={total}/></section>}
...
function Golden0509Report(){
  ...
  const municipalProfile=municipalProfiles["0509"];
  ...
  function print(){document.title=`${reportCode(definition)} · ${definition.title}`;window.print();}
  return <main className="report-shell">
    ...
    <div className="report-document">
      <Sheet definition={definition} page={1} total={total} cover><div className="report-cover-top"><Brand/><span>{definition.vault}</span></div><div className="report-cover-main"><small>{definition.eyebrow}</small><h1>{definition.title}</h1><p>{definition.subtitle}</p><div className="report-cover-rule"/><div className="report-cover-project">{brand.candidatePhotoUrl?<img src={brand.candidatePhotoUrl} alt=""/>:<i/>}<div><small>{common.campaignType.toUpperCase()}</small><b>{candidateName}</b><span>{common.municipality} · {common.department}</span></div>{brand.partyLogoUrl?<img className="party" src={brand.partyLogoUrl} alt={brand.partyName||"Partido político"}/>:<i/>}</div></div><div className="report-cover-meta"><div><span>Documento</span><b>{reportCode(definition)}</b></div><div><span>Generado</span><b>{generatedAt||"Preparando fecha…"}</b></div><div><span>Clasificación</span><b>{definition.vault==="CAMPAIGN VAULT"?"USO INTERNO Y CONFIDENCIAL":"FUENTES PÚBLICAS TRAZABLES"}</b></div></div></Sheet>
