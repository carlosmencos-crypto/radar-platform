...
export function V70CanonicalRichMunicipality() {
  ...
  return <>
    <section id="historico" className="section historical-section exportable include-print">
      ...
      <div className="continuity-panel"><div><span>PADRÓN 2011 → 2023</span><b>{registerGrowth === null ? "No publicado" : `${registerGrowth >= 0 ? "+" : "−"}${formatInteger(Math.abs(registerGrowth))} personas`}</b><small>{registerGrowthShare === null ? "Universos oficiales conservados por año" : `${registerGrowthShare >= 0 ? "+" : "−"}${formatPercent(Math.abs(registerGrowthShare))} en doce años`}</small></div><div><span>COINCIDENCIAS NOMINALES</span><ul>{model.politicalTrajectories.slice(0, 4).map((item) => <li key={`continuity-${item.name}`}>{item.name} · {item.years.join(" / ")}</li>)}{!model.politicalTrajectories.length ? <li>No se detectaron nombres repetidos en los registros incorporados.</li> : null}</ul></div><p>Las coincidencias corresponden a nombres en candidaturas y concejos documentados. No confirman identidad ni afiliación vigente.</p></div>
      <div className="political-intelligence-grid">
        ...
        <article className="trajectory-card">
          ...
          <div className="trajectory-list">{model.politicalTrajectories.map((person) => <div key={person.name}><span>{person.elections} ELECCIONES</span><div><b>{person.name}</b><p>{person.route}</p><small>{person.caution}</small></div></div>)}{!model.politicalTrajectories.length ? <p>No se detectaron nombres repetidos en los registros incorporados. Esto no demuestra que no exista trayectoria pública.</p> : null}</div>
          <p className="method-note">Regla nacional: reúne las trayectorias conservadas y las coincidencias de nombre completo entre candidaturas y concejos incorporados del municipio. Distingue candidatura de cargo adjudicado y conserva año y organización. No une nombres aproximados ni confirma identidades; la cobertura depende de los registros disponibles.</p>
        </article>
      </div>
