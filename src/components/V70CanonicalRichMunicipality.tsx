  ...
  return <>
    <section id="historico" className="section historical-section exportable include-print">
      ...
      <div className="history-panel">
        <article className="history-winner"><span>ALCALDÍA · {history.year}</span><b>{history.winner ?? "No publicado"}</b><small>{history.winnerVotes === null ? "Votos no publicados en la capa histórica" : `${formatInteger(history.winnerVotes)} votos`}</small></article>
        <article><span>SEGUNDO LUGAR</span><b>{history.runnerUp ?? "No publicado"}</b><small>{history.runnerUpVotes === null ? "Dato no publicado" : `${formatInteger(history.runnerUpVotes)} votos`}</small></article>
        <article className="history-quality"><span>Participación</span><b>{formatPercent(history.turnout)}</b><small>{history.votesCast === null ? "Dato no publicado" : `${formatInteger(history.votesCast)} votos emitidos`}</small></article>
        <article><span>MARGEN</span><b>{formatInteger(history.marginVotes)}</b><small>votos frente al segundo lugar</small></article>
      </div>
    ...
    <section className="section territory-section exportable include-print">
      ...
      <div className="territory-kpis"><article><small>COMUNIDADES</small><b>{formatInteger(model.communities)}</b><span>Núcleo electoral</span></article><article><small>AGRUPACIONES</small><b>{formatInteger(model.territorialGroups)}</b><span>Organización TSE</span></article><article><small>REGISTROS DE PADRÓN</small><b>{formatInteger(model.registered2023)}</b><span>Universo detallado 2023</span></article><article><small>CENTROS DE VOTACIÓN</small><b>{formatInteger(model.centers)}</b><span>{formatInteger(model.jrv)} JRV</span></article></div>
      <div className="community-directory">
        ...
        <label className="community-search"><span>Buscar comunidad</span><input value={communitySearch} onChange={(event) => setCommunitySearch(event.target.value)} placeholder="Escribe una comunidad…" /></label>
        <div className="community-table"><div><b>Comunidad</b><b>Electores 2023</b><b>Edad promedio</b><b>Cobertura</b></div>{filteredCommunities.slice(0, 80).map((item) => <article key={item.community_normalized}><span>{item.community_label}</span><b>{formatInteger(item.elector_count)}</b><span>{item.average_age_base === null ? "No publicada" : formatDecimal(item.average_age_base)}</span><em>{stateLabel(item.coverage_band)}</em></article>)}</div>
