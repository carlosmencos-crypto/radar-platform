  ...
  return <>
    <section id="historico" className="section historical-section exportable include-print">
      ...
      <div className="history-workspace">
        ...
        <div className="history-ranking"><div className="history-ranking-head"><b>Resultado completo</b><span>Porcentaje sobre votos por organización</span></div>{history.results.map((result) => <div className="history-row" key={`${history.year}-${result.party}-${result.rank}`}><span>{result.rank}</span><b>{result.party}</b><i><em style={{ width: `${Math.max(0, Math.min(100, result.share * 100))}%`, background: electoralPartyColor(result.party) }} /></i><strong>{formatInteger(result.votes)}</strong><small>{formatPercent(result.share)}</small></div>)}{!history.results.length ? <p className="trace-note">La fuente histórica no publicó el ranking completo para este corte.</p> : null}</div>
      </div>
      ...
      <div className="political-intelligence-grid">
        <article className="council-card">
          ...
          {council ? <><div className="seat-summary"><div><strong>{council.total}</strong><span>cargos o escaños documentados</span></div><div className="seat-bar">{council.groups.map((group) => <i key={group.party} title={`${group.party}: ${group.seats}`} style={{ width: council.titularTotal ? `${group.seats / council.titularTotal * 100}%` : "0%", background: electoralPartyColor(group.party) }} />)}</div></div><div className="seat-legend">{council.groups.map((group) => <span key={group.party}><i style={{ background: electoralPartyColor(group.party) }} /><b>{group.party}</b>{group.seats}</span>)}</div><div className="council-list">{council.members.map((member) => <div key={`${member.office}-${member.name}`}><span>{member.office}</span><b>{member.name}</b><em>{member.party}</em></div>)}</div>{!council.members.length ? <p>La fuente disponible para {historyYear} publica distribución de escaños, pero no el listado nominal completo.</p> : null}</> : <div className="canonical-vault-notice"><span>NO PUBLICADO</span><h3>Integración nominal no disponible</h3><p>RADAR no asigna cargos sin adjudicación oficial documentada para este año.</p></div>}
