...
export async function downloadActivityPng(activity: ActivityLike, brand: ActivityPngBrand = {}) {
  ...
  if (brand.partyLogoUrl) { try { const logo = await loadImage(brand.partyLogoUrl); fillRoundedRect(context, 872, 52, 142, 142, 71, "#FFFFFF"); const ratio = Math.min(112 / logo.width, 112 / logo.height); context.drawImage(logo, 943 - logo.width * ratio / 2, 123 - logo.height * ratio / 2, logo.width * ratio, logo.height * ratio); } catch { /* el PNG conserva identidad textual */ } }
