...
const areaConfig = {
  ...
  "estrategia-legal": {
    ...
    categories: ["Requisitos", "Actas", "Documentos", "Plantillas"],
  },
} as const;
...
function RecordsWorkspace({ config }: { config: (typeof areaConfig)[keyof typeof areaConfig] }) {
  ...
  const legalCandidateCategory = candidate ? form.category.replace(`${candidate[0]}:`, "") : "";
  const modal = open ? <div className="agenda-modal" role="dialog" aria-modal="true"><form className="simple-campaign-modal finance-modal" onSubmit={submit}><header><div><small>{config.moduleKey === "finanzas" ? "CONTROL FINANCIERO GENERAL" : config.moduleKey === "medios" ? "BANCO OFICIAL" : candidate ? candidate[0] : "LEGAL"}</small><h2>{config.moduleKey === "finanzas" ? "Nuevo movimiento" : config.moduleKey === "medios" ? "Nueva carga" : candidate ? legalCandidateCategory : config.add.replace(/^\+\s*/, "")}</h2></div><button type="button" onClick={() => setOpen(false)}>×</button></header><div>
    ...
    <label className="wide"><span>{config.moduleKey === "medios" ? "Nombre de la pieza" : config.moduleKey === "finanzas" ? "Concepto" : candidate ? "Nombre del archivo" : "Título"} *</span><input autoFocus required value={form.title} onChange={(event) => setForm({ ...form, title: event.target.value })} placeholder={candidate ? legalCandidateCategory : config.moduleKey === "legal" ? "Ej. Entrega del expediente del candidato" : undefined} /></label>
  ...
  if (config.moduleKey === "finanzas") {
    ...
    const exportExcel = () => {
      const campaign = `CAMPAÑA · ${brand.candidateName} · ${brand.partyName || "Campaña municipal"} · Elección municipal 2027`;
    ...
    const exportBudgetExcel = (record: CampaignModuleRecord) => {
      const brandTitle = "RADAR · INTELIGENCIA ELECTORAL";
      const campaignLine = `CAMPAÑA · ${brand.candidateName} · ${brand.partyName || "Campaña municipal"} · Elección municipal 2027`;
      const workbook = createRadarXlsx([
        { name: "Resumen", brandTitle, campaignLine, title: record.title, description: record.details || "Presupuesto específico", headers: ["Indicador", "Monto o estado", "Detalle"], rows: [["Presupuesto inicial", amount(record), "Monto aprobado para este control"], ["Ingresos", selectedIncome, "Ingresos registrados"], ["Egresos", selectedExpense, "Egresos registrados"], ["Disponible", amount(record) + selectedIncome - selectedExpense, "Presupuesto inicial + ingresos − egresos"], ["Afecta presupuesto general", record.payload?.affects_general !== false ? "Sí" : "No", record.payload?.affects_general !== false ? "Se consolida automáticamente" : "Se mantiene independiente"]], widths: [30, 22, 58], currencyColumns: [1] },
        { name: "Movimientos", brandTitle, campaignLine, title: `Movimientos · ${record.title}`, description: "Ingresos y egresos vinculados a este expediente", headers: ["Tipo", "Concepto", "Categoría", "Fuente", "Responsable", "Actividad", "Monto", "Fecha"], rows: selectedBudgetMovements.map((movement) => [movement.category, movement.title, movement.details || "—", String(movement.payload?.source || "—"), String(movement.payload?.responsible || "—"), activityName(movement.payload?.related_activity_id), amount(movement), new Date(movement.created_at).toLocaleDateString("es-GT")]), widths: [12, 34, 22, 20, 26, 30, 18, 14], currencyColumns: [6] },
      ]);
