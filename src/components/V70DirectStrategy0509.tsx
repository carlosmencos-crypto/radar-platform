...
function StrategyContent() {
  ...
  const next = useMemo(() => milestones
    .map((item) => ({ ...item, time: new Date(item.date.includes("T") ? item.date : `${item.date}T23:59:00-06:00`).getTime() }))
...
export function V70DirectStrategy0509() {
  ...
  return <MunicipalityProvider consumer={consumer}>
    <V70DirectShell0509 active="estrategia" eyebrow="ESTRATEGIA" topbarTitle="San José / Puerto San José · Escuintla" accountRole="Dirección de campaña">
      <StrategyContent />
    </V70DirectShell0509>
