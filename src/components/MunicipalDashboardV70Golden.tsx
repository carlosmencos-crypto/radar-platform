...
function GoldenIntelligenceShell() {
  const { consumer, municipality_code, municipality_name, department_name } = useMunicipalityContext();
  ...
  const activeTypes = useMemo(() => {
    ...
    if (layers.electoral) types.add("tse_voting_center");
    if (layers.schools) types.add("school");
    if (layers.territory) types.add("populated_place");
    if (layers.health) types.add("health_facility");
    return types;
  }, [layers]);
