...
export function ClientsWorkspace({snapshot,action,busy}: Props) {
 const [params,setParams]=useSearchParams(); const [query,setQuery]=useState(""); const filter=params.get("estado")??"all"; const setFilter=(value:string)=>setParams(value==="all"?{}:{estado:value}); const [code,setCode]=useState(params.get("municipio")??"");
 const [form,setForm]=useState<""|"assign"|"member"|"end"|"limit"|"reactivate"|"purge">("");
 ...
 useDismissibleDialog(Boolean(code),()=>setCode(""),busy);
 ...
 async function submit(event:FormEvent<HTMLFormElement>) {
  ...
  if(form==="purge")op="purge_campaign";
  ...
  {code&&<div className="client-backdrop" onMouseDown={e=>{if(e.target===e.currentTarget&&!busy)setCode("");}}><section className="client-drawer" role="dialog" aria-modal="true" aria-label="Ficha del municipio"><header><div><small>{municipality?.department_name} · {code}</small><h2>{municipality?.municipality_name}</h2></div><button aria-label="Cerrar ficha" disabled={busy} onClick={()=>setCode("")}>×</button></header>
   <div className="client-body">{!form?<>
    ...
    {campaign&&<><div className="client-heading"><div><small>CLIENTE</small><h3>{String(campaign.name)}</h3><p>{campaign.status==="active"?"Campaña activa":"Campaña pausada · información conservada"}</p></div><span className="client-pill">{members.length} / {String(account?.seat_limit??"Sin cupo configurado")}</span></div>
     ...
     <details className="client-danger-zone"><summary>Eliminar campaña y comenzar desde cero</summary><p>Elimina los datos privados y libera el municipio. La inteligencia municipal se mantiene.</p><button disabled={busy} onClick={()=>open("purge")}>Eliminar campaña definitivamente</button></details>
    </>}
   </>:<form className="superadmin-form" onSubmit={submit}><button type="button" onClick={()=>setForm("")} disabled={busy}>← Volver a la ficha</button><h3>{{assign:"Asignar municipio",member:member?"Cambiar acceso":"Agregar usuario",end:"Finalizar campaña",limit:"Ajustar cupo",reactivate:"Reactivar campaña",purge:"Eliminar campaña definitivamente"}[form]}</h3>
    {form==="assign"&&<><label>Nombre interno del cliente o campaña<input name="name" required placeholder={`Campaña ${municipality?.municipality_name}`} /></label><p className="client-field-help">Este nombre identifica el contrato en RADAR. El nombre del candidato, partido y sus imágenes se configuran dentro del municipio.</p><label>Nombre del administrador<input name="display_name" required /></label><label>Correo del administrador<input name="email" type="email" required /></label><label>Cupo de usuarios<select name="seat_limit" defaultValue="10"><option value="10">10 cuentas · administrador incluido</option><option value="15">15 cuentas · administrador incluido</option></select></label><label>Vigencia hasta<input type="date" name="valid_until" min={new Date().toISOString().slice(0,10)} required /></label><p>El municipio quedará reservado para este cliente. Si no tiene cuenta, recibirá un enlace para crear su contraseña. Los fiscales temporales no consumen cuentas.</p></>}
    ...
    {form==="purge"&&<><div className="client-danger-notice"><strong>Esta acción no se puede deshacer desde RADAR.</strong><p>Se eliminarán la campaña, sus registros privados, contrato e historial asociado. Se revocarán sus accesos. No se borran las cuentas personales ni la inteligencia del municipio.</p></div><label>Escribe ELIMINAR {code}<input name="confirmation" required pattern={`ELIMINAR ${code}`} autoComplete="off" /></label><label className="client-check"><input type="checkbox" name="acknowledge" value="yes" required/>Entiendo que esta campaña no podrá reactivarse.</label><p>Quedará únicamente una constancia técnica del borrado, sin copia de los datos eliminados. Los respaldos externos, si existen, siguen su propia retención.</p></>}
    ...
    <button className="superadmin-primary" type="submit" disabled={busy}>{busy?"Procesando…":form==="assign"?"Asignar y enviar acceso":form==="purge"?"Eliminar definitivamente":"Confirmar"}</button>
   </form>}</div>
  </section></div>}
 </div>;
}
